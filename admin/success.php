...
function CreateThumbs($src, $dst, $width, $height, $crop=0){
  ...
  // resize
  if($crop){
  if($w < $width or $h < $height) return "L'image est trop petite!";
  ...
  if($w < $width and $h < $height) return "l'image est très grande!";
...
//require 'includes/pictures.php';
  ...
  $trajet = $_POST['trajet'];
   $nbr_place_dispo = $_POST['nbr_place_dispo'];
  $prix = $_POST['prix'];
  ...
  //adding trajets
  $queryaddtrajets = "INSERT INTO trajets(trajet,nbr_place_dispo, description, prix, id_image, nom_image,date)
  VALUES ('$trajet',$nbr_place_dispo,'$description','$prix', '$idtrajets', '$img_title','$datetime')";

if ($connection->query($queryaddtrajets) === TRUE ) {
          echo "<div class='center-align'>
                 <h5 class='green-text'>Merci, votre offre de covoiturage est ajoutée !</h5>
                 </div><br><br>";
